...
function loadFromUrl() {
  ...
  const legacy = decodeHash(globalThis.location.hash);
...
function applyResults(data) {
  ...
  if (note) note.textContent = 'Standard FIRE number (4% row): ' + formatInr(data.swrRows[4].corpus);
...
function recalc() {
  ...
  if (linkEl) linkEl.value = globalThis.location.href;
}
...
function init() {
  ...
  if (share) share.value = globalThis.location.href;
}
